function [Ireg,Bx,By,Bz,Fx,Fy,Fz,transformedMatrix] = register_volumes(Imoving,Istatic,Options)
...
%   Options.NumofIterations : Number of iterations for Rigid and Similarity
% Notes,
...
% Process inputs
defaultoptions=struct('Similarity',[],'Registration','NonRigid','MaxRef',[],'Verbose',2,'SigmaFluid',4,'Alpha',4,'SigmaDiff',1,'Interpolation','Linear','NumofIterations',10);
...
elseif(strcmpi(Options.Registration(1),'S'))
    ...
    % Parameter scaling of the Translation, Rotation, Resize
    scale=[ 1 1 1 1 1 1 .01 ];
...
for refine_itt=1:2
    ...
    optim=struct('GradObj','off','GoalsExactAchieve',1,'Display','off','MaxIter',1000,'MaxFunEvals',1000,'TolFun',1e-14,'DiffMinChange',1e-6);
    if(Options.Verbose>0), optim.Display='iter'; end
    numberofIterations = Options.NumofIterations;
%     sc = x(7);
    for jj = 1:numberofIterations
        x(1:6) = fminsearch(@(x)affine_registration_error(x(1:6),scale(1:6),ISmoving,ISstatic,type_affine),x(1:6),optim);
%         sc = fminsearch(@(sc)affine_registration_error([x(1:6) sc], scale, ISmoving, ISstatic, type_affine),sc,optim);
        x = fminsearch(@(x)affine_registration_error(x, scale, ISmoving, ISstatic, type_affine),x,optim);
    end
%     x(7) = sc;
    %     Gradient Descent optimization functions do not work !
...
elseif(strcmpi(Options.Registration(1),'S'))
    % Make the rigid transformation matrix
    M=make_transformation_matrix(x(1:3),x(4:6), [x(7) x(7) x(7)]);
else
