 ...
 */
  ...
  void transformvolume_error(double **Args) {
#endif
    double *Isize_d, *Isize_d2, *mean, *mean2, *A, *Iin, *Iin2, *ThreadOut, *ThreadID, *moded, *Ncount;
    int Isize[3]={0, 0, 0};
    int Isize2[3]={0, 0, 0};
    int x, y, z;
    double *Nthreadsd;
    ...
    moded=Args[10]; mode=(int)moded[0];
	Ncount=Args[11]; 
    ...
    Isize2[0] = (int)Isize_d2[0];
    Isize2[1] = (int)Isize_d2[1];
    Isize2[2] = (int)Isize_d2[2];
    ...
    for (z=offset; z<Isize2[2]; z=z+Nthreads) 
    {
        ...
        for (y=0; y<Isize2[1]; y++)
        {
            ...
            for (x=0; x<Isize2[0]; x++) 
            {
                ...
                /* Index coordinate current pixel in static image volumeix */
                indexI=mindex3(x, y, z, Isize2[0], Isize2[1]);

                Tlocalx = dcomp0;
                Tlocaly = dcomp1;
                Tlocalz = dcomp2;
                
				if ( Tlocalx < 0 || Tlocalx >= Isize[0] || Tlocaly < 0 || Tlocaly >= Isize[1] || Tlocalz < 0 || Tlocalz >= Isize[2] )
					continue;
				
                I=interpolate_3d_double_gray(Tlocalx, Tlocaly, Tlocalz, Isize, Iin, cubic, black);
//				if (I>100) printf("Warning: very large interpolated value (%10.5f)\n", I);
//            	printf("The average of row\n");

                /* Calculate squared difference */
                sqd+=(I-Iin2[indexI])*(I-Iin2[indexI]);
				
				Ncount[offset] = Ncount[offset] + 1;
            }
        }
    }
...
void mexFunction( int nlhs, mxArray *plhs[],
    ...
    double *Iin, *Iin2, *Ierror, *M, *ThreadOut, *Igradient, *moded, *Ncount, *Npixel;
    ...
    double Isize_d2[3]={0, 0, 0};
    double Isize_min[3]={0, 0, 0};
    const mwSize *dims;
    ...
    if(nlhs==3) /* Also gradient needed */
    {
        /* Create output array */
    ...
    Isize_d2[0] = (double)dims[0]; Isize_d2[1] = (double)dims[1]; Isize_d2[2] = (double)dims[2];
    ...
    Iin=(double *)mxGetData(prhs[0]);		// moving image
    Iin2=(double *)mxGetData(prhs[1]);		// fixed image
	...
	Ncount = (double *)malloc( Nthreads * sizeof(double) );
//	Npixel = (double *)malloc( sizeof(double) );
	
    /* Assign pointer to output. */
    Ierror = (double *)mxGetData(plhs[0]);
    Npixel = (double *)mxGetData(plhs[1]);
    
    /* Center of the volume */
    mean[0]=Isize_d[0]/2;  mean[1]=Isize_d[1]/2;  mean[2]=Isize_d[2]/2;
    mean2[0]=Isize_d2[0]/2;  mean2[1]=Isize_d2[1]/2;  mean2[2]=Isize_d2[2]/2;
    ...
    for (i=0; i<Nthreads; i++) {
		...
		ThreadArgs1[1]=Isize_d2;
        ThreadArgs1[2]=mean;
        ThreadArgs1[3]=mean2;
        ThreadArgs1[4]=A;
        ThreadArgs1[5]=Iin;
        ThreadArgs1[6]=Iin2;
        ThreadArgs1[7]=ThreadOut;
        ThreadArgs1[8]=ThreadID[i];
        ThreadArgs1[9]=Nthreadsd;
        ThreadArgs1[10]=moded;
        ThreadArgs1[11]=Ncount;
    ...
    Ierror[0]/= Isize_d2[0]*Isize_d2[1]*Isize_d2[2]; // (*Npixel); 
    if(nlhs==3) /* Also gradient needed */
    {
        for (j=0; j<12; j++)
        {
            ...
            Igradient[j]/= Isize_d2[0]*Isize_d2[1]*Isize_d2[2]*delta; // (*Npixel)*delta; 
        }
    }
